    ...
    if (query.filter["cashbackPayment"]) {
      try {
        if (query.filter["cashbackPayment"] == "incomplete") {
          ...
          query.filter = {
            ...query.filter,
            $or: [{ cashbackPayment: { $exists: false } }, { "cashbackPayment.status": "incomplete" }],
          };
        } else {
    ...
    if (query.filter["interestPayment"]) {
      try {
        if (query.filter["interestPayment"] == "incomplete") {
          ...
          query.filter = {
            ...query.filter,
            $or: [{ interestPayment: { $exists: false } }, { "interestPayment.status": "incomplete" }],
          };
        } else {
